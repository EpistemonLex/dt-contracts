...
- **Memory Sovereignty**: Every model must use 'ConfigDict(frozen=True)' to leverage Pydantic V2's native Rust optimizations and prevent OOM on 1GB Edge devices.
...
2. **TDD**: Write tests in 'tests/' verifying that models are 'frozen' and that malformed telemetry is rejected.
